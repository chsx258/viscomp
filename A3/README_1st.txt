...
A. REFERENCE SOLUTIONS
  ...
  1. The directory reference_results/ contains a set of screenshots of what
     a fully-functional interface looks like, along with intermediate stages 
     of the inpainting algorithm. Use these as a guide if you cannot run the
     supplied reference binary executables.
  2. The directory binary_ref_soln contains a binary compiled for macOS that you
     can run from a terminal window using the command 
	      ./viscomp-gui.osx -- --use-gui
	 Depending on your version of the operating system, you may need to change
	 the file permissions to allow execution, and you may need to allow the
	 system to run this binary using the System Preferences->Security & Privacy
	 preference pane. IMPORTANT NOTE: This binary expects a fully-functional
	 Kivy specification file, to be located in binary_ref_soln/kv/viscomp.kv.
	 The supplied viscomp.kv file is incomplete, however, as it does not include
	 a run button and does not handle crosshair updates. You will need to 
	 complete this file before the binary executable can be used for running
	 the inpainting algorithm.
  3. I am also supplying a binary executable compiled for linux, but your 
     mileage will vary in running that code because of possible system 
	 incompatibilities with variouls libraries, etc installed on your system.
  
  If you cannot get (2) or (3) to work easily, I recommend you rely on the
  supplied screenshots for testing your code.  
...
E. EXTENDING THE STARTER CODE
  ...
  1. The starter code is (almost) ready to run the inpainting
     algorithm: once you implement the image-loading functions
     in file algorithm.py and implement the Run button in the Kivy interface, 
	 you should be able to load images and run iterations of the algorithm. 
  2. The only problem with the starter code implementation as it stands, is 
     that the patch priorities are not computed correctly because code uses
...
2. IMPORTANT: 
  ...
  * All your UI-related code should go in the file 
    code/inpaintingui/viewer.py 
    code/inpaintingui/widgets.py 
...
3. GENERAL STRUCTURE -- GUI
  ...
  to this file in order to implement the functionalities requested in A3
  ...
  inpaintingui/widgets.py. Read this file next and try to understand it well. 
  You will need to write one of its methods.

  To load/save images in the Inpainting object, the code uses
  a class called InpaintingControl. This 'sits' in between the RootWidget class
  and the Inpainting class. The RootWidget class you need to complete will 
  have to call one of the methods in this file.
...
4. GENERAL STRUCTURE -- Inpainting algorithm
  ...
  variables from/to files, and for doing inpainting.
...
5. FILES IN THE DIRECTORY code
   ...
   kv/viscomp.py	
			...
			about this language beyond what is in this file already. Note that
			the specification file is written for Kivy version 1.9 but has
			been tested with Kivy 2.0 as well.
